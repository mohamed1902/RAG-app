...
class PGVectorProvider(VectorDBInterFace):
    ...
    def __init__(self, db_client, default_vector_size: int = 786, distabce_method: str = None, index_threshold: int = 100):

        self.db_client = db_client
        self.default_vector_size = default_vector_size
        self.index_threshold = index_threshold

        if distabce_method == DistanceMethodEnums.COSINE.value:
            distabce_method = PgVectorDistanceMethodEnums.COSINE.value
        elif distabce_method == DistanceMethodEnums.DOT.value:
            distabce_method = PgVectorDistanceMethodEnums.DOT.value
        
        self.pgvector_table_prefix = PgVectorTableSchemeEnums._PREFIX.value
        self.distabce_method = distabce_method
                ...
                async with session.begin():
                    create_sql = sql_text(
                        f'CREATE TABLE {collection_name} ('
                            ...
                            f'{PgVectorTableSchemeEnums.METADATA.value} json DEFAULT \'{{}}\', '
        ...
        async with self.db_client() as session:
            async with session.begin():
                ...
                create_idx_sql = sql_text(
                                            ...
                                            f'USING {index_type} ({PgVectorTableSchemeEnums.VECTOR.value} {self.distabce_method})'
                                          )
    ...
    async def insert_one(self, collection_name, text, vector, metadata = None, record_id = None):
        ...
        async with self.db_client() as session:
                async with session.begin():
                    ...
                    await session.ececute(insert_sql,{
                        'text': text,
        ...
        async with self.db_client() as session:
                async with session.begin():
                    for i in range(0, len(texts), batch_size):
                        ...
                        for  _text, _vector, _metadata, _record_id in zip(batch_texts, batch_vectors, batch_metadata, batch_records_ids):
    ...
    async def search_by_vector(self, collection_name, vector, limit):
        ...
        is_collection_existed = await self.is_collection_existed(collection_name=collection_name)
        if not is_collection_existed:
            self.logger.error(f"can not search for record in a non-existed collection: {collection_name}")
            return False
